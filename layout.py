...
def build_sidebar():
    return html.Div(style={'width': '100%'}, children=[
        ...
        card([
            ...
            html.Button('▶ Executar Análise Histórica', id='btn-run-upload', n_clicks=0,
                        style={'width': '100%', 'background': BLUE, 'color': ACCENT,
                               ...
                               'marginBottom': '4px'}),
            html.Div('Carrega base auxiliar + boletos históricos',
                     style={'fontSize': '9px', 'color': MUTED, 'textAlign': 'center',
                            'marginBottom': '12px', 'fontStyle': 'italic'}),
        ]),

        # Carteira Nova — painel separado
        card([
            html.Div('🔍 Carteira Nova', style={'fontWeight': '600', 'marginBottom': '4px',
                                                'fontSize': '14px', 'color': WARN}),
            html.Div('Opcional — boletos a pagar para análise de cobertura PCR',
                     style={'fontSize': '9px', 'color': MUTED, 'marginBottom': '8px',
                            'fontStyle': 'italic'}),
            dcc.Upload(id='upload-cart', multiple=False,
                       style={'border': f'2px dashed {WARN}', 'borderRadius': '8px',
                              'cursor': 'pointer', 'marginBottom': '6px'},
                       children=html.Div([
                           html.Div('⬆', style={'fontSize': '20px', 'color': WARN}),
                           html.Div('3. Carteira Nova', style={'fontSize': '12px',
                                                               'fontWeight': '600'}),
                           html.Div('carteira_atual.csv',
                                    style={'fontSize': '10px', 'color': MUTED}),
                       ], style={'textAlign': 'center', 'padding': '12px'})),
            html.Div(id='cart-status',
                     style={'fontSize': '11px', 'color': WARN, 'marginBottom': '10px'}),
            html.Button('🔍 Analisar Carteira Nova', id='btn-run-cart', n_clicks=0,
                        style={'width': '100%', 'background': BLUE, 'color': WARN,
                               'border': f'1px solid {WARN}', 'borderRadius': '8px',
                               'padding': '9px', 'fontWeight': '700', 'cursor': 'pointer',
                               'fontFamily': "'Space Grotesk',sans-serif", 'fontSize': '12px',
                               'marginBottom': '4px'}),
            html.Div('Analisa cobertura PCR e CNPJs sem histórico',
                     style={'fontSize': '9px', 'color': MUTED, 'textAlign': 'center',
                            'fontStyle': 'italic'}),
        ]),
...
def build_welcome():
    return html.Div([
        ...
        html.Div('Faça o upload dos três CSVs na barra lateral e clique em ▶ Executar.',
